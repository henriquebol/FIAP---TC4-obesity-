...
def run_analise_page():
    ...
    with tab2:
        st.markdown("#### 🔹 Análise dos Boxplots das variáveis")
        try:
            col1, col2 = st.columns(2, gap="large")
            with col1:
                numerical_features = ['Idade', 'Altura(m)', 'Peso(kg)', 'FCVC']
                for feature in numerical_features:
                    img_comp = Image.open(f"graphs/box_plot_{feature}.png")
                    st.image(img_comp, use_container_width=True) #caption="Comparação de Acurácia entre os Modelos"
            with col2:
                numerical_features = ['NCP', 'CH2O', 'FAF', 'TUE']
                for feature in numerical_features:
                    img_comp = Image.open(f"graphs/box_plot_{feature}.png")
                    st.image(img_comp, use_container_width=True) #caption="Comparação de Acurácia entre os Modelos"
